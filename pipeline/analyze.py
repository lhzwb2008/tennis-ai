...
def score_and_write(
    ...
    *,
    ...
    if late >= 0.4:
        problems.append(
            f"约 {int(late*100)}% 的挥拍在速度峰值时手腕仍偏在髋后，击球点容易偏晚、偏近身。"
        )
        drills.append(
            "【问题】击球点偏晚 → 【原因】引拍完成晚、启动慢 → 【训练】球过网时必须完成引拍；前脚前方放标志物，必须在标志物前击球，15球×4组。"
        )
    if elbow is not None:
        strengths.append(f"击球附近持拍肘角中位约 {elbow:.0f}°（2D 估计，有透视误差）。")

    if not problems:
        problems.append("未触发硬性阈值问题；建议结合教练看片确认拍面与击球点。")
    if not drills:
        drills.append("保持当前框架，增加不同落点的移动击球，再复测步幅与还原。")

    if source == "original":
        caveats = [
            "指标来自单机位 2D 姿态，不是实验室三维测角。",
            "没有球与拍面检测，击球帧用持拍手腕速度峰值近似，可能比真实触球略晚。",
            "背面机位难以直接判断击球点前后距离与拍面开合。",
        ]
        if view == "back":
            caveats.append("肘角、引拍深度受透视压缩，步幅和左右开立相对更可信。")
    else:
        caveats = [
            "指标来自叠加视频上的 2D 姿态，不是实验室三维测角。",
            "没有球与拍面检测，击球帧用持拍手腕速度峰值近似。",
            "画面已烧入原平台骨架，可能干扰关键点；拍面翻转无法定量。",
        ]
